...
def save_all_features(args, all_features, all_labels, all_ids):
    ...
    columns = ['ID'] + feature_columns + ['Fraud_Types']
    
    labels_df = pd.DataFrame(labels_and_ids, columns=columns)

    # Save the DataFrame to a CSV file
    labels_df.to_csv(os.path.join(args.feature_path, 'all_features_with_labels.csv'), index=False)
...
def save_mean_features(args, mean_features_by_label):
    ...
    mean_features_df.index.name = 'Fraud_Types'
