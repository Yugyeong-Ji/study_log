...
# Load the dataset
data = pd.read_csv("/workspace/Dataset/FSI/train_preprocessed.csv")
...
for i in range(len(data.columns)):
    print(f"{i}th column: {data.columns[i]}")
...
if len(data.columns) >= 1:
    ...
    print("Value counts for the Fraud_Type column:") 
    ...
    def sample_group(group):
        return group.sample(n=10000, random_state=42, replace=True)  # Use replace=True to handle cases with less than 10,000 samples

    sampled_data = data.groupby(data.columns[fraud_type_index], group_keys=False).apply(sample_group).reset_index(drop=True)
    
    # Save the sampled data to a new CSV file
    sampled_data.to_csv("/workspace/Dataset/FSI/train_sample_preprocessed.csv", index=False)
