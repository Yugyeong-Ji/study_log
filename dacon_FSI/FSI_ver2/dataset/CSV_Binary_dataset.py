...
class CSVBinaryDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.args.test:
            ...
            input = torch.tensor(self.features[idx], dtype=torch.float32).to(self.args.device)
            label = torch.tensor(self.labels[idx], dtype=torch.long).to(self.args.device)
    ...
    def preprocess(self):
        ...
        if self.args.test:
            ...
            sampled_data = self.synthetic_data.groupby(self.synthetic_data.columns[-2]).apply(sample_group).reset_index(drop=True)
